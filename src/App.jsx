 ...
 */
const METRICS = [
 { tag: "GestiCalc — Gesture Accuracy", val: "92%+", desc: "184/200 gestures correct · CPU-only webcam", },
 { tag: "Logistics Platform Latency", val: "85%", desc: "DB latency reduced via Zod + Redis caching", },
 { tag: "SLA Compliance", val: "99.9%", desc: "platform uptime · resolved 25+ incidents", },
 { tag: "Conversion Tracking", val: "+20%", desc: "accuracy via PostHog + Google Ads funnels", },
];

const SKILLS = [
 { title: "Languages & Core", items: [["Python","90"],["Java / TypeScript","78"],["SQL (PostgreSQL)","80"],["C++","72"]] },
 { title: "Backend & Infra", items: [["Kafka (Event Streaming)","80"],["AWS (EC2, S3, Lambda)","68"],["Redis (Cache-Aside)","76"],["Docker / Terraform / CI-CD","72"]] },
 { title: "Analytics & PM Tools", items: [["PostHog","85"],["Google Ads","78"],["JIRA (Advanced Roadmaps)","82"],["Tableau / Grafana / Prometheus","72"]] },
 { title: "AI & Engineering", items: [["CrewAI / Agentic AI","80"],["ML Pipelines","70"],["React / Next.js / Node.js","76"],["Distributed Tracing / Zod","70"]] },
];
...
const PROJECTS = [
 ...
 disclaimer: "Live sensor data is only visible when an Arduino/ESP32 board is physically connected and transmitting. The demo shows UI and layout without hardware.",
...
const AI_CARDS = [
 { title:"Multi-Agent Task Orchestration", desc:"CrewAI-based system where specialized agents (Researcher, Analyst, Writer) collaborate autonomously to complete complex multi-step tasks without human intervention.", status:"building" },
 { title:"Emotion-Aware Interfaces", desc:"Exploring real-time emotion detection via webcam to adapt UI behavior and content delivery based on user emotional state using CNN-based classification.", status:"done", github:"https://github.com/sneha-attu/emotion-detection-text" },
 { title:"Rubik's Cube Solver", desc:"Interactive React web app implementing layer-by-layer and advanced cube-solving algorithms with 3D visualization and move-by-move explanation engine.", status:"done", demo:"https://rubiks-cube-solver-delta.vercel.app/" },
];
...
const AWARDS = [
 ...
 { title:"Infosys Global Hackathon 2025 — Top 10", org:"Infosys + Cloud Native Computing Foundation (CNCF)", year:"2025", desc:"Ranked in the Top 10 at the Infosys Global Hackathon 2025 powered by Infosys and CNCF. Built an innovative solution for global good, competing against thousands of participants worldwide.", color:"#3b82f6", cert:"Verified at shortner.infy.com/verify" },
 { title:"ONEST Hackathon 2025 — 3rd Place", org:"Deshpande Startups · Hubballi", year:"2025", desc:"Secured 3rd Place at the ONEST Hackathon 2025 at Deshpande Startups, Hubballi — building impactful solutions for real-world challenges in education and skilling.", color:"#f97316", cert:"Certificate of Participation" },
 { title:"PMx 2026 — Product Management Expedition", org:"IIT Guwahati · Udgam 2026 · Unstop", year:"2026", desc:"Participated in PMx 2026 — The Product Management Expedition organised by IIT Guwahati as part of Udgam 2026. Represented Gogte Institute of Technology, Belgaum.", color:"#a78bfa", cert:"Verified via Unstop" },
];

const ACH_METRICS = [
 { val:"92%+", title:"GestiCalc Accuracy", desc:"184/200 gestures · CPU-only webcam" },
 { val:"85%", title:"DB Latency Reduced", desc:"Zod governance + Redis caching" },
 { val:"+20%", title:"Conversion Accuracy", desc:"PostHog + Google Ads funnels" },
 { val:"99.9%", title:"SLA Maintained", desc:"25+ critical issues resolved" },
 { val:"+15%", title:"SDLC Velocity", desc:"Sprint & JIRA optimisation" },
 { val:"3rd", title:"ONEST Hackathon 2025", desc:"Deshpande Startups · Hubballi" },
 { val:"Top 10", title:"Infosys Global Hackathon", desc:"Infosys + CNCF · 2025" },
 { val:"4+", title:"Hackathons Participated", desc:"ISRO · Infosys · ONEST · IIT Guwahati" },
];
...
const LEADERSHIP = [
 ...
 { icon:role:"Leader & Former Volunteer", org:"U&I NGO — Education for Underprivileged Children", color:"#34d399", desc:"Led a team of volunteers at U&I, an NGO focused on providing quality education to underprivileged children. Coordinated teaching sessions, managed volunteer scheduling, and drove community outreach initiatives." },
];
...
#neural-canvas{position:fixed;inset:0;z-index:0;opacity:0.4;pointer-events:none}
nav{position:fixed;top:0;left:0;right:0;z-index:200;display:flex;align-items:center;justify-content:space-between;padding:1rem 3rem;background:rgba(2,4,8,0.92);backdrop-filter:blur(24px);border-bottom:1px solid var(--b2);transition:padding .3s}
nav.slim{padding:.65rem 3rem}
.nav-brand{font-weight:700;font-size:1rem;color:var(--cyan);letter-spacing:-.01em;cursor:pointer;text-decoration:none;flex-shrink:0}
.nav-links{display:flex;gap:1.4rem;list-style:none}
.nav-links a{font-family:var(--mono);font-size:.68rem;color:var(--muted2);text-decoration:none;letter-spacing:.04em;text-transform:uppercase;transition:color .2s}
.nav-links a:hover{color:var(--cyan)}
.nav-contact{background:transparent;color:var(--cyan);font-family:var(--mono);font-weight:600;font-size:.78rem;padding:.45rem 1.1rem;border-radius:6px;border:1px solid rgba(56,189,248,0.4);cursor:pointer;flex-shrink:0;transition:background .2s,box-shadow .2s}
...
.hbar{display:block;width:24px;height:2px;background:var(--cyan);border-radius:2px;transition:transform .3s,opacity .3s}
...
.mob-drawer{position:fixed;top:0;right:-100%;width:min(300px,80vw);height:100vh;background:rgba(5,8,15,0.98);backdrop-filter:blur(24px);border-left:1px solid var(--b2);z-index:250;transition:right .35s cubic-bezier(.4,0,.2,1);padding:5rem 2rem 2rem}
.mob-drawer.open{right:0}
.mob-links{display:flex;flex-direction:column;gap:.2rem}
.mob-link{font-family:var(--mono);font-size:.85rem;color:var(--muted2);text-decoration:none;letter-spacing:.06em;text-transform:uppercase;padding:.75rem 1rem;border-radius:8px;transition:background .2s,color .2s;border:1px solid transparent}
...
.lbl{font-family:var(--mono);font-size:.7rem;color:var(--cyan);letter-spacing:.12em;text-transform:uppercase;margin-bottom:.6rem}
.ttl{font-size:clamp(1.9rem,3.5vw,2.8rem);font-weight:700;letter-spacing:-.03em;line-height:1.1;margin-bottom:1rem}
...
.btn-p{background:var(--cyan);color:#000;font-weight:600;font-size:.875rem;padding:.75rem 1.6rem;border-radius:8px;border:none;cursor:pointer;font-family:var(--font);text-decoration:none;display:inline-block;transition:box-shadow .25s,transform .2s}
.btn-p:hover{box-shadow:0 0 28px var(--cglow);transform:translateY(-2px)}
.btn-s{background:transparent;color:var(--text);font-weight:500;font-size:.875rem;padding:.75rem 1.6rem;border-radius:8px;border:1px solid var(--b2);cursor:pointer;font-family:var(--font);text-decoration:none;display:inline-block;transition:border-color .2s,color .2s}
...
.hero-badge{display:inline-flex;align-items:center;gap:.5rem;font-family:var(--mono);font-size:.7rem;color:var(--emerald);border:1px solid rgba(52,211,153,.3);padding:.3rem .9rem;border-radius:99px;margin-bottom:1.4rem}
.bdot{width:6px;height:6px;background:var(--emerald);border-radius:50%;animation:blink 2s infinite}
@keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}
.hero-name{font-size:clamp(3rem,6vw,5.5rem);font-weight:700;letter-spacing:-.04em;line-height:1;margin-bottom:.8rem}
...
#skills{padding:5rem 0;background:linear-gradient(180deg,var(--bg) 0%,var(--bg2) 100%)}
.sg-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1.5rem;margin-top:2rem}
.sg{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.6rem;transition:border-color .3s,transform .3s}
...
.sg-icon{font-size:1.3rem}.sg-title{font-weight:600;font-size:.95rem}
...
.exp-tl{position:relative;margin-top:2rem}
.exp-tl::before{content:'';position:absolute;left:11px;top:0;bottom:0;width:1px;background:linear-gradient(180deg,var(--cyan),var(--violet),transparent)}
.exp-item{position:relative;padding-left:3rem;margin-bottom:3rem}
.exp-dot{position:absolute;left:0;top:6px;width:24px;height:24px;border-radius:50%;background:var(--bg);border:2px solid var(--cyan);display:flex;align-items:center;justify-content:center}
.exp-dot span{width:8px;height:8px;background:var(--cyan);border-radius:50%}
.exp-card{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.6rem 2rem;transition:border-color .3s}
.exp-card:hover{border-color:rgba(56,189,248,.25)}
.exp-meta{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:.6rem}
.exp-role{font-weight:700;font-size:1.05rem;color:var(--text)}
.exp-period{font-family:var(--mono);font-size:.7rem;color:var(--muted);background:var(--surface2);padding:.25rem .7rem;border-radius:4px;border:1px solid var(--b2);white-space:nowrap}
...
.etag{font-family:var(--mono);font-size:.65rem;color:var(--violet);background:var(--vdim);padding:.2rem .6rem;border-radius:4px}
#projects{padding:5rem 0;background:linear-gradient(180deg,var(--bg2) 0%,var(--bg) 100%)}
.proj-grid{display:grid;grid-template-columns:1fr 1fr;gap:1.5rem}
.pcard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);overflow:hidden;display:flex;flex-direction:column;transition:border-color .3s,box-shadow .3s,transform .3s}
...
.pcb{background:linear-gradient(135deg,var(--surface2) 0%,rgba(56,189,248,.05) 100%);padding:2rem;border-bottom:1px solid var(--b2);position:relative;overflow:hidden}
.pcard.full .pcb{border-bottom:none;border-right:1px solid var(--b2);padding:2.5rem}
.pcb::before{content:'';position:absolute;top:-40%;right:-10%;width:250px;height:250px;background:radial-gradient(circle,rgba(56,189,248,.08),transparent 70%);border-radius:50%}
...
.psl{font-family:var(--mono);font-size:.65rem;color:var(--cyan);letter-spacing:.1em;text-transform:uppercase;margin-bottom:.5rem}
.arch{display:flex;align-items:center;flex-wrap:wrap;gap:.3rem;margin-bottom:1rem}
.anode{background:var(--card);border:1px solid var(--b2);color:var(--text);padding:.25rem .6rem;border-radius:4px;font-family:var(--mono);font-size:.68rem;white-space:nowrap}
.aarr{color:var(--cyan);font-size:.8rem}
.ptags{display:flex;flex-wrap:wrap;gap:.4rem;margin-bottom:1rem}
.ptag{font-family:var(--mono);font-size:.65rem;color:var(--muted2);background:var(--surface2);padding:.2rem .55rem;border-radius:4px;border:1px solid var(--b2)}
.plinks{display:flex;gap:.75rem;margin-top:auto}
.plink{font-family:var(--mono);font-size:.72rem;color:var(--cyan);text-decoration:none;display:flex;align-items:center;gap:.3rem;border:1px solid rgba(56,189,248,.25);padding:.35rem .8rem;border-radius:5px;transition:background .2s}
...
#system-design{padding:5rem 0}
.sd-grid{display:grid;grid-template-columns:1fr 1fr;gap:2rem;margin-top:2rem}
...
.sd-sh{font-family:var(--mono);font-size:.65rem;color:var(--violet);letter-spacing:.1em;text-transform:uppercase;margin-bottom:.5rem}
...
.pro{color:var(--emerald)}.con{color:#f87171}
...
.cnode{background:var(--surface2);border:1px solid var(--b2);font-family:var(--mono);font-size:.68rem;color:var(--text);padding:.25rem .6rem;border-radius:4px;white-space:nowrap}
.cdesc{font-size:.78rem;color:var(--muted2)}
#ai-experiments{padding:5rem 0;background:linear-gradient(180deg,var(--bg) 0%,var(--bg2) 100%)}
.ai-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:1.2rem;margin-top:2rem}
.aicard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.5rem;position:relative;overflow:hidden;transition:all .3s}
.aicard::before{content:'';position:absolute;inset:0;background:radial-gradient(circle at 80% 20%,rgba(167,139,250,.06),transparent 60%);pointer-events:none}
...
#leadership{padding:5rem 0;background:linear-gradient(180deg,var(--bg2) 0%,var(--bg) 100%)}
.lead-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1.4rem;margin-top:2rem}
.leadcard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.6rem;position:relative;overflow:hidden;transition:border-color .3s,transform .3s,box-shadow .3s}
...
#achievements{padding:5rem 0;background:var(--bg2)}
.awards-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:1.3rem;margin-top:2rem}
.acard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.5rem;position:relative;overflow:hidden;transition:border-color .3s,transform .3s,box-shadow .3s}
...
.aw-icon{display:flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:10px;background:rgba(255,255,255,.04);border:1px solid var(--b2);font-size:1.3rem;flex-shrink:0}
.aw-year{font-family:var(--mono);font-size:.62rem;color:var(--muted);background:var(--surface2);border:1px solid var(--b2);padding:.2rem .6rem;border-radius:4px;white-space:nowrap}
...
.ach-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.3rem}
.achcard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.3rem;display:flex;align-items:flex-start;gap:.9rem;transition:border-color .3s}
.achcard:hover{border-color:rgba(56,189,248,.2)}
.ach-ico{font-size:1.4rem;flex-shrink:0}
.ach-val{font-family:var(--mono);font-size:1.3rem;font-weight:700;color:var(--cyan);line-height:1;margin-bottom:.2rem}
.ach-tit{font-size:.85rem;font-weight:600;margin-bottom:.15rem}
.ach-desc{font-size:.75rem;color:var(--muted2)}
.cert-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1.2rem;margin-top:1.5rem}
.certcard{background:var(--card);border:1px solid var(--b2);border-radius:var(--r);padding:1.3rem;display:flex;flex-direction:column;gap:.6rem;transition:all .3s}
...
function useNeural() {
 ...
 nodes.push({ x: Math.random()*W, y: Math.random()*H, vx: (Math.random()-.5)*.35, vy: (Math.random()-.5)*.35, r: Math.random()*1.4+.6, pulse: Math.random()*Math.PI*2, color: Math.random() > 0.75 ? '167,139,250' : '56,189,248' });
 }
 };
 ...
 if (d<130) { ctx.beginPath(); ctx.moveTo(nodes[i].x,nodes[i].y); ctx.lineTo(nodes[j].x,nodes[j].y); ctx.strokeStyle=`rgba(56,189,248,${0.13*(1-d/130)})`; ctx.lineWidth=.5; ctx.stroke(); }
 }
 nodes.forEach(n => {
 const pr = Math.max(0.4, n.r+Math.sin(n.pulse)*0.5);
 ctx.beginPath(); ctx.arc(n.x,n.y,pr,0,Math.PI*2); ctx.fillStyle=`rgba(${n.color},${0.55+Math.sin(n.pulse)*0.2})`; ctx.fill();
 ctx.beginPath(); ctx.arc(n.x,n.y,pr*3,0,Math.PI*2); ctx.fillStyle=`rgba(${n.color},0.04)`; ctx.fill();
 });
...
const go = (id, e) => { e && e.preventDefault(); document.getElementById(id)?.scrollIntoView({ behavior:"smooth", block:"start" }); };
...
function Hero() {
 ...
 <div className="fade">
 <div className="hero-badge">Software Engineer · AI Builder · CS 2026</div>
 ...
 <div className="mc-head"><span className="mc-tag">{m.tag}</span></div>
...
function Skills() {
 ...
 <div className="sg-head"><span className="sg-title">{g.title}</span></div>
 <div className="sbars">
...
function Projects() {
 return (
 <section id="projects" style={{padding:"5rem 0",background:"linear-gradient(180deg,var(--bg2) 0%,var(--bg) 100%)"}}>
 <div className="ctr">
 ...
 {p.metrics&&<div className="pm-row">{p.metrics.map(([v,l])=><div key={l}><div className="pm-val">{v}</div><div className="pm-lbl">{l}</div></div>)}</div>}
 </div>
 <div className="pbody">
...
function SystemDesign() {
 ...
 <div className="sd-title">{c.title}</div><div className="sd-sub">{c.sub}</div>
...
function AchievementsAndCerts() {
 ...
 <div className="aw-head"><span className="aw-year">{a.year}</span></div>
...
function ContactForm() {
 ...
 setStatus("sending");
 try {
 const res = await fetch("https://formspree.io/f/xbdzqaqj",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(form)});
 if (res.ok){setStatus("success");setForm({name:email:subject:message:""});}
 else setStatus("error");
 } catch{setStatus("error");}
 };
 const iStyle={width:"100%",background:"var(--surface2)",border:"1px solid var(--b2)",borderRadius:"8px",padding:".7rem 1rem",color:"var(--text)",fontFamily:"var(--font)",fontSize:".9rem",outline:"none",transition:"border-color .2s"};
 const lStyle={fontFamily:"var(--mono)",fontSize:".65rem",color:"var(--muted)",letterSpacing:".08em",textTransform:"uppercase",display:"block",marginBottom:".4rem"};
 ...
 <div><label style={lStyle}>Your Name *</label><input name="name" value={form.name} onChange={handleChange} placeholder="Jane Doe" style={iStyle} onFocus={focus} onBlur={blur}/></div>
 <div><label style={lStyle}>Your Email *</label><input name="email" type="email" value={form.email} onChange={handleChange} placeholder="jane@example.com" style={iStyle} onFocus={focus} onBlur={blur}/></div>
 </div>
...
function Contact() {
 ...
 <span className="cc-icon"><svg width="26" height="26" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/></svg></span>
 <span className="cc-lbl">GitHub</span><span className="cc-val">github.com/sneha-attu</span>
 </a>
 </div>
 <ContactForm/>
